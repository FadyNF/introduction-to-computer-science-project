...
bool loginCode() {
	...
	for (int i = 1; i <= 5; i++) {
		...
		if (checkAdmin(userName, password)) {
			...
			if (i == 5) {
				//if finished fifth try << block user
				system("cls");
				cout << "Error , You are Blocked !! :(\n";
				exit(0);
			}
			else {
				// if didn't finish fifth try << reprompt username and password
...
bool checkAdmin(string UserName, int Password) {
	...
	int pass = 1234;
	//if prompted matches correct UN and Pass return true
	if (name == UserName && pass == Password) {
		return true;
	}
	//if prompted doesn't matches correct UN and Pass return false
...
void generateQuestions() {
	...
	//open question and answer files and fill arrays with file content
